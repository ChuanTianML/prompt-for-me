...
function buildSuggestionInput(args, currentEvents, historicalRecords, config) {
  ...
  return Object.freeze({
    ...
    excludedCandidates: excluded.slice(-config.candidateCount),
  })
}
...
function systemPrompt(candidateCount) {
  ...
  return [
    ...
    "Write every candidate in the user's voice as a request, instruction, correction, decision, or answer addressed to the coding agent.",
